...
void setup(){
  ...
  var cvs = document.getElementById("displaycanvas");
  rad = min(cvs.clientWidth / 2, cvs.clientHeight / 2) * (1 / 1.2);
  ...
  center = new PVector(window.innerWidth / 2, window.innerHeight / 2);


  PGraphics bg = createGraphics(window.innerWidth, window.innerHeight, RGB);
  bg.beginDraw();
