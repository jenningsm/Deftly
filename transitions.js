...
function fullSweep(maxSpeed, accel){
  return function(dir, next){
    sweep(this, (dir ? 0 : 1), (dir ? 1 : 0), (dir ? maxSpeed : 0), (dir ? 0 : maxSpeed), maxSpeed, accel, accel, next);
  }
}

function sweep(element, start, stop, startSpeed, stopSpeed, maxSpeed, accel, decel, next){
  var slowat;
  if(decel === 0){
    slowat = stop;
  } else {
...
/* this put in handle the case when the max speed is not reached (slowat needs to be different in such a case) but it currently does not work  
  ...
  sweepUtil(element, (start < stop ? true : false), stop, maxSpeed, accel, decel, start, (accel === 0 ? maxSpeed : startSpeed), slowat, next);
}
